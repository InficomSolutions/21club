...
function countDownTimer({ GAME_TYPE_ID }) {
  ...
  countDownInterval1 = setInterval(function () {
    const { minute, seconds,seconds1, seconds2 } = getTimeMSS(countDownDate);
    if (GAME_TYPE_ID !== "1") {
      $(".TimeLeft__C-time div:eq(1)").text(minute);
    } else {
      $(".TimeLeft__C-time div:eq(1)").text("0");
    }
    $(".TimeLeft__C-time div:eq(2)").text(seconds);
  ...
  countDownInterval2 = setInterval(() => {
    ...
    if (seconds == 0 && seconds1 == 0 && seconds2 <= 5) {
      if (clicked) {
        if (check_volume == "on") {
          playAudio1();
        }
    ...
    if (seconds == 0 && seconds1 == 5 && seconds2 == 5) {
      if (clicked) {
        if (check_volume == "on") {
          playAudio2();
        }
